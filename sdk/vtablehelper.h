...
namespace VTABLE {
		...
		int MFI_GetVtblOffset(void* mfp) {
			...
			if (*addr++ == 0xFF) {
				if (*addr == 0x60) {
					...
					return *((unsigned int*)++addr) / sizeof(void*);
				} else if (*addr == 0x20) {
					return 0;
				} else {
					return -1;
	...
	template<>
	struct MFI_Impl<1 * sizeof(void*)> // simple ones
	{
		template<class MFP>
		static inline void GetFuncInfo(MFP mfp, MemFuncInfo& out) {
	...
	template<>
	struct MFI_Impl<2 * sizeof(void*)> // more complicated ones!
	{
	...
	template<>
	struct MFI_Impl<3 * sizeof(void*)> // WOW IT"S GETTING BIGGER OMGOMOGMG
	{
	...
	template<>
	struct MFI_Impl<4 * sizeof(void*)> // THE BIGGEST ONE!!!1GABEN
	{
		template<class MFP>
		static inline void GetFuncInfo(MFP mfp, MemFuncInfo& out) {
			out.vtblindex = MFI_GetVtblOffset(*(void**)&mfp);
...
#error Unsupported compiler
	...
	template<class X>
	inline void GetFuncInfo(X mfp, MemFuncInfo& out) {
...
namespace TOOLS {
	...
	int GetVtableIndex(T vfuncPtr) {
		VTABLE::MemFuncInfo info {};
		VTABLE::GetFuncInfo(vfuncPtr, info);
	...
	int GetVtableIndex(X* pInstance, Y vfuncPtr) {
		VTABLE::MemFuncInfo info {};
		VTABLE::GetFuncInfo(pInstance, vfuncPtr, info);
		return info.vtblindex;
	}
} // namespace TOOLS
